...
def record_mouse_events(closing_event,
    ...
    with mouse.Events() as events:
        ...
        while time() < expected_end_time:
            ...
            #Check if certain time is lapsed as per 'time_steps' (based on fps)
            if loop_start_time - last_event_start_time >= time_steps:
                ...
                if event is None:       #idle time
                    ...
                    if idle_time == False:
                    ...
                    elif time() - idle_time_start > max_idle_time:
                        ...
                        #count idle time
                        idle_time = time() - idle_time_start
                
                else:
                    idle_time = 0
                    #print(f'Received event {event}')
                    
                    #Mouse movedto new position
                    if str(type(event)) == "<class 'pynput.mouse.Events.Move'>": #Change it later
                        #print('Mouse moved to x, y = ', event.x, event.y)
                        x,y = event.x, event.y
                        left,right = 0,0
                    
                    #Press mouse button   
                    elif str(type(event)) == "<class 'pynput.mouse.Events.Click'>":
                        #print('x,y, button, pressed = ', event.x, event.y, event.button, event.pressed)
                        x,y  = event.x, event.y
                        if event.pressed:
                        ...
                        #Ignoring 
                        # 1. scroll event, and 
                        # 2. event.pressed == False, 
                        # 3. mouse.Button.middle, 
                        # 4. unknown button

                #data.append([loop_start_time - record_start_time,x,y,left,right])
                writer.writerow([loop_start_time - record_start_time,x,y,left,right])
    #Summary
    duration = time() - record_start_time      
    print(f'{duration=}')
